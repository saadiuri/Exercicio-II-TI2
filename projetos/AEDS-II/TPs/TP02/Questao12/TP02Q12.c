...
int comparar_pokemons_bolha(Pokemon p1, Pokemon p2)
{
    if (p1.id < p2.id)
        return -1;
    else if (p1.id > p2.id)
...
void bubble_sort(Pokemon array[], int n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        ...
        {
            if (comparar_pokemons_bolha(array[j], array[j + 1]) > 0)
            {
                trocar_pokemon(&array[j], &array[j + 1]);
                swapped = true;
            }
        }
        // Se nenhuma troca ocorreu na passagem, o array já está ordenado
        if (!swapped)
            break;
    }
}
...
int main()
{
    ...
    // Ordenar os Pokémons selecionados pelo id usando Bubble Sort
    bubble_sort(selected_pokemons, selected_count);

    // Abrir o arquivo de log
    FILE *log_file = fopen("bolha.txt", "w");
    if (log_file == NULL)
    {
